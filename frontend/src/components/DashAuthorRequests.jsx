...
export default function DashAuthorRequests() {
    ...
    const fetchRequests = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/user/author-requests`,
          {
            credentials: 'include',
          }
        );
  ...
  const handleDecision = async (userId, decision) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/user/${decision}-author/${userId}`,
        {
          method: 'PUT',
          credentials: 'include',
        }
  ...
  return (
    <div className='table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500'>
      ...
      {currentUser.isAdmin && requests.length > 0 ? (
        ...
        <p className='text-center'>There are no pending requests right now.</p>
      )}
    </div>
  );
}
