...
export default function DashUsers() {
  ...
  const handleToggleAdmin = async (user) => {
    if (user._id === currentUser._id) {
      return; // don't let an admin accidentally demote themselves here
    }
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/user/set-admin/${user._id}`,
        {
          method: 'PUT',
          credentials: 'include',
      ...
      if (res.ok) {
        setUsers((prev) =>
          prev.map((u) => (u._id === user._id ? data : u))
        );
      } else {
        console.log(data.message);
      }
